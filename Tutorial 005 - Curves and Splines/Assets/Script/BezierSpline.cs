...
public class BezierSpline : MonoBehaviour {
	...
	public int CurveCount {
		get {
			return (points.Length - 1) / 3; //exactly what it sounds like. Number of bezier curves.
		}
	}
	...
	public void AddCurve () {
		...
		Array.Resize(ref points, points.Length + 3); //ref means passing by reference. resize to allow 3 more points.
		point.x += 1f;
	...
	private void EnforceMode (int index) {
		...
		//Now which point should we adjust? When we change a point's mode, it is either a point 
		//in between curves or one of its neighbors. When we have the middle point selected, we 
		//can just keep the previous point fixed and enforce the constraints on the point on the 
		//opposite side. If we have one of the other points selected, we should keep that one 
